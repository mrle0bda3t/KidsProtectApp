...
public class DatabaseHelper extends SQLiteOpenHelper {
    private static final String TAG = "Kidsapp.Database";

    private static final String DB_NAME = "Kidsapp";
...
//        db.execSQL("CREATE INDEX idx_access_block ON access(block)");
    ...
    // Tạo bảng dữ liệu Blacklist = ID , idbl ( id Blacklist )
    private void createTableBlackList(SQLiteDatabase db) {
        Log.i(TAG, "Creating Blacklist table");
        db.execSQL("CREATE TABLE blacklist (" +
                " ID INTEGER PRIMARY KEY AUTOINCREMENT" +
                ", idbl TEXT NOT NULL" +
                ", apdung INTEGER NOT NULL" +
                ");");
    }

    // Bảng dữ liệu url để chặn = ID, url , url , idbl ( id Blacklist )
    // TODO : Sử dụng giá trị url thay vì xác định bằng id url -> cần cải thiện
    // Chấp nhận sử dụng phương án sử dụng url xác định bằng giá trị url và ID Blacklist
    ...
    private void createAppBlock(SQLiteDatabase db) {
        Log.i(TAG, "Creating appblock table");
        db.execSQL("CREATE TABLE appblock (" +
                " ID INTEGER PRIMARY KEY AUTOINCREMENT" +
    ...
    // Cập nhật mã đồng bộ thông tin truy cập
    public void updatesync(String idsync) {
        ...
        try {
            ...
            try {
                ...
                Log.i(TAG, "Tiến hành cập nhật mã đồng bộ");
                if (rows == 0) {
                    Log.i(TAG, "Tiến hành thêm mã đồng bộ");
                    if (db.insert("sync", null, cv) == -1)
                        Log.e(TAG, "Lỗi thêm mã đồng bộ");
                }
                db.setTransactionSuccessful();
            } finally {
                db.endTransaction();
    ...
    public void updateAccess(String url) {
        ...
        try {
            ...
            try {
                ...
                db.update("access", cv, "daddr LIKE ?", new String[]{"%" + url + "%"});
    ...
    public void deleteBlacklist(String id) {
        ...
        try {
            ...
            db.beginTransactionNonExclusive();
            try {
                db.delete("blacklist", "idbl = ?", new String[]{id});
    ...
    public void deleteUrl(String url) {
        ...
        try {
            ...
            db.beginTransactionNonExclusive();
            try {
                db.delete("url", "url = ?", new String[]{url});
    ...
    public void deleteUrlbyIdBl(String idbl) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransactionNonExclusive();
            try {
                db.delete("url", "idbl = ?", new String[]{idbl});
    ...
    /// xoá url theo url và idbl
    public void deleteUrlWithIdBlackList(String idchan, String idbl) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransactionNonExclusive();
            try {
                db.delete("url", "idchan = ? and idbl = ?", new String[]{idchan, idbl});
    ...
    /// Thêm url
    public void updateUrl(String url, String idbl, int apdung) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransactionNonExclusive();
    ...
    /// Cập nhật áp dụng url
    public void updateApDungUrl(String idbl, int apdung) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransactionNonExclusive();
            try {
                ContentValues cv = new ContentValues();
                cv.put("apdung",apdung);
                if (db.update("url",cv, "idbl=?", new String[]{idbl}) == -1)
                    Log.e(TAG, "Insert access failed");
                db.setTransactionSuccessful();
            } finally {
                db.endTransaction();
    ...
    /// cập nhật blacklist auto cho áp dụng là 1
    public void updateBlackList(String idbl) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransactionNonExclusive();
    ...
    /// cập nhật áp dụng blacklist
    public void updateApDungBlacklist(String idbl, int apdung){
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransactionNonExclusive();
            try {
                ContentValues cv = new ContentValues();
                cv.put("apdung",apdung);
                db.update("blacklist",cv, "idbl=?",new String[]{idbl});
                db.setTransactionSuccessful();
            } finally {
                db.endTransaction();
            }
    ...
    public void insertAppBlock(String nameapp, String timeStart, String timeEnd) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransactionNonExclusive();
            try {
                ...
                cv.put("block",0);
                if (db.insert("appblock", null, cv) == -1)
                    Log.e(TAG, "Insert access failed");
                db.setTransactionSuccessful();
            } finally {
                db.endTransaction();
    ...
    public void updateAppBlock(String nameapp, String timeStart, String timeEnd) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransactionNonExclusive();
            try {
                ...
                cv.put("timeStart", timeStart);
                cv.put("timeEnd", timeEnd);
                db.update("appblock", cv, "nameapp = ?", new String[]{nameapp});
                db.setTransactionSuccessful();
            } finally {
                db.endTransaction();
            }
    ...
    public void updateBlockInApp(String nameapp, int block) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransactionNonExclusive();
            try {
                ContentValues cv = new ContentValues();
                Log.d("Tên app :"+nameapp, "giá trị block : "+ String.valueOf(block));
                cv.put("block", block);
                db.update("appblock", cv, "nameapp = ?", new String[]{nameapp});
                db.setTransactionSuccessful();
            } finally {
    ...
    public boolean CheckIsAppBlockAlreadyInDB(String nameapp) {
        lock.readLock().lock();
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            Cursor cursor = db.query("appblock", null, "nameapp = ?", new String[]{nameapp}, null, null, null);
    ...
    public boolean CheckIsAppIsBlock(String nameapp) {
        lock.readLock().lock();
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            Cursor cursor = db.query("appblock", null, "nameapp = ? AND block = 1", new String[]{nameapp}, null, null, null);
            if (cursor.getCount() <= 0) {
                cursor.close();
                return false;
            }
    ...
    public void deleteAppBlock(String nameapp) {
        lock.writeLock().lock();
        try {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransactionNonExclusive();
            try {
                db.delete("appblock", "nameapp = ?", new String[]{nameapp});
                db.setTransactionSuccessful();
            } finally {
                db.endTransaction();
            }
    ...
    /// Kiểm tra xem data có ở trong bảng nào đó không
    ...
    /// kiểm tra idbl và url có ở trong bảng url không
    public boolean CheckIsBlAlreadyInURLorNot(String idbl, String url) {
        lock.readLock().lock();
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            Cursor cursor = db.query("url", null, "idbl=? AND url=?", new String[]{idbl, url}, null, null, null);
            if (cursor.getCount() <= 0) {
                cursor.close();
                return false;
            }
    ...
    /// Kiểm tra xem url đang truy cập có contains url trong bảng url không
    public boolean CheckIsDataAlreadyContainsUrlorNot(String TableName, String fieldValue) {
        lock.readLock().lock();
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            Cursor cursor = db.query(TableName, null, "instr('" + fieldValue + "',url) > 0 AND apdung=1", null, null, null, null);
            if (cursor.getCount() <= 0) {
                cursor.close();
                return false;
            }
    ...
    public Cursor getUrl() {
        ...
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            return db.query("url", null, null, null, null, null, null);
        } finally {
            lock.readLock().unlock();
        }
    }

    public Cursor getAppBlock() {
        lock.readLock().lock();
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            return db.query("appblock", null, null, null, null, null, null);
        } finally {
            lock.readLock().unlock();
        }
    }
